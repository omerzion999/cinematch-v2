...
def test_clean_poster_path_normalizes_nan_and_empty_strings():
    ...
    from app.poster import clean_poster_path

    assert clean_poster_path(math.nan) is None
    assert clean_poster_path("") is None
    assert clean_poster_path("/abc123.jpg") == "/abc123.jpg"


def test_resolve_poster_path_prefers_catalog_value(monkeypatch):
    import app.poster as poster_module

    def fail_if_called(*args, **kwargs):
        raise AssertionError("search_tv_show should not be called when catalog has a poster_path")

    monkeypatch.setattr(poster_module, "search_tv_show", fail_if_called)

    pick = {"title": "Breaking Bad", "poster_path": "/catalog.jpg", "start_year": 2008}
    assert poster_module.resolve_poster_path(pick) == "/catalog.jpg"
...
def test_resolve_poster_path_falls_back_to_tmdb_when_catalog_value_missing(monkeypatch):
    ...
    assert poster_module.resolve_poster_path(pick) == "/from-tmdb.jpg"


def test_resolve_poster_path_returns_none_when_tmdb_has_no_match(monkeypatch):
    import app.poster as poster_module

    monkeypatch.setattr(poster_module, "search_tv_show", lambda title, year=None: None)

    pick = {"title": "Dead to Me", "poster_path": "", "start_year": 2019}
    assert poster_module.resolve_poster_path(pick) is None
